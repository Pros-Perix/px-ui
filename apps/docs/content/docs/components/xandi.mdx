...
```tsx
// Response type returned by your fetchResponse function
...
type MessageType = "text" | "markdown" | "jobs";

// Union type for structured data payloads
type MessageData = JobsData | null;
...
When your API returns structured data (like jobs), set the appropriate `type` and `data` in your response:
