...
The File Upload system gives you two ways to build file upload experiences: use `FileUploadSimple` for a ready-to-use component, or combine `useFileUpload` with the `FileUpload.*` primitives for fully custom UIs.
...
This example uses `FileUploadSimple`, which includes state management, validation, and optional upload support.
...
`FileUploadSimple` supports three layout variants.
...
// Dropzone (default)
<FileUploadSimple variant="dropzone" />

// Button
<FileUploadSimple variant="button" buttonText="Upload Files" multiple />

// Compact
<FileUploadSimple variant="compact" accept=".pdf,.doc,.docx" />
````
