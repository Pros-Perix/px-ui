...
import { FileUploadProgressDemo } from "../../../src/components/file-upload-progress-demo";
...
<FileUpload.Root>
  <FileUpload.Dropzone hideDefaultContent>
    <div className="text-center">
      <UploadIcon className="text-ppx-neutral-8 mx-auto size-12" />
      <p className="text-ppx-sm text-ppx-neutral-12 mt-2">
...
<FileUpload.Root accept="image/*">
  <FileUpload.Trigger variant="primary">Upload Image</FileUpload.Trigger>
  <FileUpload.ItemList>
    {(files) =>
      files.map((file) => (
        <FileUpload.Item key={file.id} file={file}>
...
For uploading to S3 with signed URLs, use the `useS3Upload` hook. Pass your API functions directly:

```tsx
import { FileUpload, useS3Upload, type FileUploadFile } from "@px-ui/core";
...
function S3FileUpload() {
  ...
  const { uploadFiles, uploadedFiles } = useS3Upload({
    ...
    onUploadError: (error, file) => {
      Toast.ApiErr(error, "Error uploading file");
    },
  });
...
```tsx
// Pass your existing API functions directly
const { uploadFiles, uploadedFiles, isUploading } = useS3Upload({
  getSignedUrl: GetSignedUrl, // or PostInboxSignedUrl
  uploadToS3: Upload,
  forResume: false, // Optional
  onUploadComplete: (file) => {}, // Optional
  onUploadError: (err, file) => {}, // Optional
  onAllUploadsComplete: (files) => {}, // Optional
});

// Returns
uploadFiles(files); // Upload multiple FileUploadFile[]
uploadFile(file); // Upload single FileUploadFile
isUploading; // boolean
uploadingFiles; // Map<id, { file, progress }>
uploadedFiles; // S3UploadedFile[]
failedFiles; // Array<{ file, error }>
reset(); // Clear state
```
...
interface S3UploadedFile {
  ...
  path: string; // S3 path without query params
  format: string; // File format from MIME type
  fullPath: string; // Full S3 URL
  size: number;
...
For faster uploads, use `useS3ParallelUpload`:

```tsx
import { useS3ParallelUpload } from "@px-ui/core";

const { uploadFiles } = useS3ParallelUpload({
  maxConcurrent: 3, // Upload 3 files at a time
  getSignedUrl,
  uploadToS3,
});
```
...
Pass your API functions directly - no wrapper needed:

```tsx
import { FileUpload, useS3Upload } from "@px-ui/core";
import { GetSignedUrl, PostInboxSignedUrl } from "../Api/Rest";
import { Upload } from "../Api/Requestor";

function MyUpload({ fromInbox = false }) {
  const { uploadFiles, isUploading } = useS3Upload({
    // Pass API functions directly
    getSignedUrl: fromInbox ? PostInboxSignedUrl : GetSignedUrl,
    uploadToS3: Upload,
    forResume: false,
  });
