...
- **`FileUploadSimple`** - Ready-to-use component that abstracts both the hook and UI primitives for quick setup
- **`FileUpload.*` + `useFileUpload`** - Composable primitives and hook for full customization

The `FileUploadSimple` component internally uses the `FileUpload.*` primitives and `useFileUpload` hook, so you get the same functionality with less boilerplate.
...
The `FileUpload.*` components are UI primitives that work with the `useFileUpload` hook. This is what `FileUploadSimple` uses internally - giving you the same power with full customization.

**Note:** When using upload functionality, pass `retryUpload` and `isUploading` from the hook to `FileUpload.Root` for complete upload support.
...
| `FileUpload.Item`          | Individual file item wrapper with status styling   |
...
| `FileUpload.ItemStatus`    | Status indicator (uploading %, success, or failed) |
| `FileUpload.ItemError`     | Error message display                              |
| `FileUpload.ItemRetry`     | Retry button for failed uploads                    |
| `FileUpload.ClearButton`   | Remove all files button                            |
| `FileUpload.ImageGrid`     | Grid layout for images                             |
| `FileUpload.ImageGridItem` | Image grid item with status overlays               |
...
| `retryUpload`     | `(id: string) => Promise<void>`       | No       | Retry upload action from hook |
...
| `isUploading`     | `boolean`                             | No       | Whether upload is in progress |
...
| Prop                 | Type      | Default          | Description                     |
| -------------------- | --------- | ---------------- | ------------------------------- |
| `uploadingText`      | `string`  | `'Uploading...'` | Text shown while uploading      |
| `showUploadingState` | `boolean` | `true`           | Show spinner and text on upload |
| `...ButtonProps`     | -         | -                | All Button component props      |
...
| `statusStyles` | `boolean`        | `true`  | Apply error styling based on status |

### FileUpload.ItemStatus

| Prop               | Type        | Default   | Description                   |
| ------------------ | ----------- | --------- | ----------------------------- |
| `successIcon`      | `ReactNode` | Checkmark | Custom success icon           |
| `uploadingContent` | `ReactNode` | `{n}%`    | Custom uploading text/element |
| `errorContent`     | `ReactNode` | `Failed`  | Custom error text/element     |
...
| ------------------- | ---------------- | ------- | ---------------------------------- |
| `file`              | `FileUploadFile` | -       | The file object (required)         |
| `showStatusOverlay` | `boolean`        | `true`  | Show uploading/error/success state |
...
- **Start with `FileUploadSimple`**: For most use cases, use `FileUploadSimple` - it abstracts both the hook and primitives
- **Use primitives for customization**: When you need full control, use `FileUpload.*` with `useFileUpload`
- **Pass all hook actions**: When using primitives, pass `retryUpload` and `isUploading` for full upload support
- **Show upload status**: Use `ItemStatus`, `ItemProgress`, `ItemError`, and `ItemRetry` for complete feedback
- **Provide clear constraints**: Pass validation options (`maxSize`, `maxFiles`, `accept`) to the hook
