...
import FormSelectBasicDemo from "../../../src/components/form-select-basic-demo.tsx";
import FormMixedDemo from "../../../src/components/form-mixed-demo.tsx";

## Overview

Form components are pre-built wrappers that combine Field components with common controls (Input, Textarea, Checkbox, Select), integrated with react-hook-form's Controller. They provide a quick way to build forms with consistent styling, validation, and error handling out of the box.
...
```tsx
import { FormInput, FormTextarea, FormCheckbox, FormSelect } from "@px-ui/forms";
```
...
Select dropdown integrated with react-hook-form. This is built on top of `SelectField` and is ideal for common dropdown use cases.
...
A form select field that integrates `SelectField` with Field and react-hook-form's Controller.

| Prop          | Type              | Default | Description                                                         |
| ------------- | ----------------- | ------- | ------------------------------------------------------------------- |
| `control` \*  | `Control`         | -       | The control object from `useForm()` hook                            |
| `name` \*     | `string`          | -       | The name of the form field. Must match a key in your form data type |
| `label` \*    | `React.ReactNode` | -       | Label text for the select field                                     |
| `description` | `React.ReactNode` | -       | Optional description text shown below the select                    |
| `placeholder` | `string`          | -       | Placeholder text displayed when no item is selected                 |
| `required`    | `boolean`         | `false` | Whether the field is required. Adds a visual indicator to the label |
| `items` \*    | `Item[]`          | -       | Items to display in the dropdown (the selected value is the item)   |
...
- For radio groups or more complex form controls (custom Select trigger/layout, grouped options, searching, etc.), use the Field components directly with react-hook-form's Controller component
