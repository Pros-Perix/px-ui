import { BlockCheckboxGroup } from "@px-ui/core";

export function BlockCheckboxGroupWithIconsDemo() {
  return (
    <BlockCheckboxGroup.Group defaultValue={["pro"]}>
      <BlockCheckboxGroup.Item value="basic">
        <BlockCheckboxGroup.Header>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="20"
            height="20"
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            strokeWidth="2"
            strokeLinecap="round"
            strokeLinejoin="round"
            className="text-ppx-neutral-11"
          >
            <path d="M18 20a6 6 0 0 0-12 0" />
            <circle cx="12" cy="10" r="4" />
          </svg>
          <BlockCheckboxGroup.Title>Basic Plan</BlockCheckboxGroup.Title>
        </BlockCheckboxGroup.Header>
        <BlockCheckboxGroup.Description>
          Perfect for individuals and small projects. Includes core features.
        </BlockCheckboxGroup.Description>
      </BlockCheckboxGroup.Item>

      <BlockCheckboxGroup.Item value="pro">
        <BlockCheckboxGroup.Header>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="20"
            height="20"
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            strokeWidth="2"
            strokeLinecap="round"
            strokeLinejoin="round"
            className="text-ppx-primary-5"
          >
            <path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2" />
            <circle cx="9" cy="7" r="4" />
            <path d="M22 21v-2a4 4 0 0 0-3-3.87" />
            <path d="M16 3.13a4 4 0 0 1 0 7.75" />
          </svg>
          <BlockCheckboxGroup.Title>Pro Plan</BlockCheckboxGroup.Title>
        </BlockCheckboxGroup.Header>
        <BlockCheckboxGroup.Description>
          For growing teams. Includes advanced features and priority support.
        </BlockCheckboxGroup.Description>
      </BlockCheckboxGroup.Item>
    </BlockCheckboxGroup.Group>
  );
}
