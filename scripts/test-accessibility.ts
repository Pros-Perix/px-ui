...
async function runAllTests(): Promise<A11yTestResult[]> {
  ...
  console.log(`Running ${COMPONENTS_TO_TEST.length} component tests concurrently...`);
  
  const testPromises = COMPONENTS_TO_TEST.map(async (component): Promise<A11yTestResult | null> => {
    try {
  ...
  return results.filter((result): result is A11yTestResult => result !== null);
}
